...
class ProductsModel
{
    ...
    {
        $query = "select t1.id_product, t1.product_name, t1.price, t1.description, 
                        t2.category, t3.subcategory_name, t1.start, t1.quantity, t1.downloadable
                         from " . $table . " as t1 
                         inner join categories as t2 on t1.id_category = t2.id_category 
                         inner join subcategories as t3 on t1.id_subcategory = t3.id_subcategory";

        $stmt = Conexion::connect()->prepare($query);
        if ($stmt->execute()) {
            return $stmt->fetchAll();
        } else "Productos no encontrados";
        $stmt->close();
    }
    ...
    public static function getProductById($data, $table)
    {
        ...
        $query = "select t1.id_product, t1.product_name, t1.price, t1.description, t2.id_category, t2.category, t3.id_subcategory, t3.subcategory_name, t1.start, t1.quantity, t1.downloadable from " . $table . " as t1 inner join categories as t2 on t1.id_category = t2.id_category inner join subcategories as t3 on t1.id_subcategory = t3.id_subcategory where id_product =" . $parseInt;
        $stmt = Conexion::connect()->prepare($query);
        $stmt->execute();
        $result = $stmt->fetch();
        return $result;
