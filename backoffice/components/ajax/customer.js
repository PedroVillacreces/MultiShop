...
$(document).on("click", '.deleteButton', function () {
    if (confirm('¿Estas seguro que desea eliminar al cliente?')) {
        ...
        $.ajax({
            ...
            if (data === "ok") {
                $(this).parent().remove();
                $("#customersRow" + id_customer).remove();
...
$(document).on('click', '.doUpdate', function () {
    ...
    if(name && surname && email && address && pc && region && phone && password && password_confirmation) {
...
$(document).on('click', '.createButton', function () {
  ...
  if(name && surname && email && address && pc && region && phone && password && password_confirmation) {
      ...
      alert('Para hacer la inserción por favor revise y rellene todos los campos del formulario');
  }
});
